 ...
 * Implements overflow-safe grid and tab row layout rules.
 */
export const LcExamMatrixSidebar: React.FC<LcExamMatrixSidebarProps> = ({
  ...
  return (
    <aside
      className={`fixed lg:sticky top-0 lg:top-20 right-0 z-40 h-full lg:h-[calc(100vh-6rem)] w-80 max-w-[90vw] bg-theme-surface border-l lg:border border-theme lg:rounded-2xl p-4 sm:p-5 shadow-xl lg:shadow-xs transition-transform duration-300 flex flex-col gap-3 ${
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-theme/50 pb-2.5 shrink-0">
        ...
        <button
          onClick={onClose}
          className="lg:hidden p-1.5 rounded-lg text-theme-secondary hover:text-theme-primary hover:bg-theme-surface-2 cursor-pointer focus-visible:ring-2 focus-visible:ring-theme-accent"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="space-y-1 shrink-0">
        <span className="text-[11px] font-semibold text-theme-secondary flex items-center gap-1">
          <Filter className="w-3 h-3 text-theme-accent shrink-0" />
        ...
        <div className="grid grid-cols-5 gap-1">
          {(['ALL', 1, 2, 3, 4] as const).map((partVal) => (
            <button
              ...
              className={`py-1 rounded-lg text-[11px] font-bold transition-all cursor-pointer truncate ${
                filterPart === partVal
      ...
      {/* Status Filters */}
      <div
        className="flex items-center gap-1.5 overflow-x-auto pb-1 shrink-0 touch-pan-x"
        style={{ scrollbarWidth: 'thin' }}
      >
        <button
          onClick={() => setFilterStatus('ALL')}
          className={`px-2.5 py-1 rounded-md text-[10px] font-semibold whitespace-nowrap transition-colors shrink-0 cursor-pointer ${
            filterStatus === 'ALL'
      ...
      <div className="pt-2 border-t border-theme/50 text-[11px] text-theme-secondary flex items-center justify-between gap-2 shrink-0 flex-wrap">
