...
import { ArrowLeft, Clock, Send, Flag, CheckCircle2, Eye, RefreshCw, Layers, Trophy, XCircle, Sparkles } from 'lucide-react';
...
export const ExamTakePage: React.FC<ExamTakePageProps> = ({ docId, mode, onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-theme-base pb-24">
              ...
              >
                    ...
                    {isSubmitted && (
                      isCorrect ? (
                        ...
                        <span className="flex items-center gap-1 text-xs font-bold text-rose-400 bg-rose-500/10 px-2.5 py-1 rounded-full border border-rose-500/20">
                          <XCircle className="w-3.5 h-3.5" /> Sai (Đã chọn: {selectedOpt || 'Bỏ trống'})
                        </span>
                      )
                    )}

                    {/* AI Explanation & Knowledge Recall Button (Works for BOTH Correct & Wrong Questions!) */}
                ...
                <div className="space-y-2.5 pt-2">
                  {q.options.map(opt => {
                    ...
                    if (isCorrectOpt) {
                      optStyle = 'bg-emerald-500/20 border-emerald-500 text-emerald-300 font-bold shadow-md shadow-emerald-500/10';
                    } else if (isUserWrongOpt) {
                      optStyle = 'bg-rose-500/20 border-rose-500 text-rose-300 font-bold shadow-md shadow-rose-500/10';
                    } else if (isSelected) {
                      optStyle = 'bg-theme-accent/20 border-theme-accent text-theme-primary font-bold shadow-md shadow-indigo-500/10';
                    }
                ...
                {/* Practice Mode Toggle Button if not submitted */}
          ...
          <div className="sticky top-32 bg-theme-surface rounded-2xl border border-theme p-5 space-y-5 shadow-xl">
            ...
            <div className="grid grid-cols-5 gap-2 max-h-[50vh] overflow-y-auto pr-1">
              {filteredMatrixQs.map(q => {
                ...
                const isWrong = isSubmitted && userAnswers[q.id] && userAnswers[q.id] !== q.correct_answer;

                let gridStyle = 'bg-theme-surface-2 hover:bg-theme-surface-3 text-theme-secondary border border-theme';
                if (isSubmitted) {
                  if (isCorrect) gridStyle = 'bg-emerald-500 text-white font-bold shadow-md shadow-emerald-500/20';
                  else if (isWrong) gridStyle = 'bg-rose-500 text-white font-bold shadow-md shadow-rose-500/20';
                  else gridStyle = 'bg-amber-500/20 text-amber-300 border border-amber-500/30';
                } else if (isFlagged) {
              ...
              {examResult ? (
                <>
                  <div className="flex items-center gap-1.5">
                    <span className="w-3 h-3 rounded-md bg-emerald-500" /> Đúng
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="w-3 h-3 rounded-md bg-rose-500" /> Sai
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="w-3 h-3 rounded-md bg-amber-500/30 border border-amber-500/50" /> Bỏ trống
                  </div>
                </>
              ) : (
                <>
      ...
      {/* AI Explanation Modal on ExamTakePage */}
      {selectedAiQuestion && (
